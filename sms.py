...
def export_data():
    ...
    data = [studentTable.item(index)['values'] for index in studentTable.get_children()]
    pandas.DataFrame(data, columns=['Id', 'Name', 'Mobile', 'Email', 'Address', 'Gender', 'DOB', 'Added Date', 'Added Time']).to_csv(url, index=False)
...
def toplevel_data(title, button_text, command):
    ...
    for i, field in enumerate(fields):
        Label(screen, text=field, font=('times new roman', 20, 'bold')).grid(row=i, column=0, padx=30, pady=15, sticky=W)
        entries[field] = Entry(screen, font=('roman', 15, 'bold'), width=24)
        entries[field].grid(row=i, column=1, pady=15, padx=10)
    
    idEntry, nameEntry, phoneEntry, emailEntry, addressEntry, genderEntry, dobEntry = entries.values()
    ttk.Button(screen, text=button_text, command=command).grid(row=len(fields), columnspan=2, pady=15)
...
def show_student():
    ...
    for data in mycursor.execute('SELECT * FROM student'):
        studentTable.insert('', END, values=data)

def delete_student():
    content_id = studentTable.item(studentTable.focus())['values'][0]
    mycursor.execute('DELETE FROM student WHERE id=%s', (content_id,))
    con.commit()
...
def connect_database():
    def connect():
        ...
        try:
            ...
            mycursor.execute('''CREATE TABLE IF NOT EXISTS student (
                                id INT NOT NULL PRIMARY KEY, name VARCHAR(30), mobile VARCHAR(10), email VARCHAR(30),
    ...
    fields = ['Host Name', 'User Name', 'Password']
    entries = {field: Entry(connectWindow, font=('roman', 15, 'bold'), bd=2) for field in fields}
    for i, (field, entry) in enumerate(entries.items()):
        Label(connectWindow, text=field, font=('arial', 20, 'bold')).grid(row=i, column=0, padx=20)
        entry.grid(row=i, column=1, padx=40, pady=20)
    
    hostEntry, usernameEntry, passwordEntry = entries.values()
    ttk.Button(connectWindow, text='CONNECT', command=connect).grid(row=len(fields), columnspan=2)
...
Label(root, font=('times new roman', 18, 'bold')).place(x=5, y=5)
ttk.Button(root, text='Connect database', command=connect_database).place(x=980, y=0)

leftFrame = Frame(root)
leftFrame.place(x=50, y=80, width=300, height=600)

buttons = [
    ('Add Student', toplevel_data, 'Add Student', 'Add', None),
    ('Search Student', toplevel_data, 'Search Student', 'Search', None),
    ('Delete Student', delete_student, None, None, None),
    ('Update Student', toplevel_data, 'Update Student', 'Update', update_data),
    ('Show Student', show_student, None, None, None)
]

for i, (text, command, *args) in enumerate(buttons):
    ttk.Button(leftFrame, text=text, width=25, command=lambda c=command, a=args: c(*a) if a[0] else c()).grid(row=i+1, column=0, pady=20)
...
#         global mycursor,con
#         try:
#             con=pymysql.connect(host='localhost',user='root',password='030206')
#             mycursor=con.cursor()
#         except:
#             messagebox.showerror('Error','Invalid Details',parent=connectWindow)
#             return
...
# sliderLabel=Label(root,font=('lucida console',28,'italic bold'),width=30)
